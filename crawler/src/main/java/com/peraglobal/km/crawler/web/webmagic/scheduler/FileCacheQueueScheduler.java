...
public class FileCacheQueueScheduler extends DuplicateRemovedScheduler implements MonitorableScheduler {
        ...
        try {
            ...
            while ((line = fileUrlReader.readLine()) != null) {
                ...
                if (lineReaded >= cursor.get()) {
                	if(line.indexOf("&datumId") != -1){
                		String[] _line = line.split("&datumId=");
                		line = _line[0];
                		datumId = _line[1];
                		queue.add(new Request(line).setDatumId(datumId));
                	}else{
    ...
    protected void pushWhenNoDuplicate(Request request, Task task) {
        ...
        if(!urls.contains(request.getUrl())){
        	queue.add(request);
        	if(request.getDatumId() != null){
        		fileUrlWriter.println(request.getUrl()+"&datumId="+request.getDatumId());
        	}else{
	...
	public void removeCash(int state,Task task) {
		...
		this.flush();
		inited.set(false);
		if(state == Page.STATE_PAGE_STOP){
			File file_url = new File(this.getFileName(fileUrlAllName));
			File file_cursor = new File(this.getFileName(fileCursor));
			if (file_url.exists()) {
				file_url.delete();
	        }
			if(file_cursor.exists()){
				file_cursor.delete();
			}
		}
	}
