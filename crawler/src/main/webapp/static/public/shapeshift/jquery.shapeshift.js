...
/*
      Plugin.prototype.enableDragNDrop = function() {
        ...
        if (options.enableDrag) {
          $container.children("." + active_class).filter(options.dragWhitelist).dragable({
            addClasses: false,
...
*/
      ...
      Plugin.prototype.destroy = function() {
        ...
        if (this.options.enableDrag) {
          $active_children.dragable('destroy');
        }
      ...
      Plugin.prototype.enableDragNDrop = function() {
    	    var $clone, $container, $placeholder, $selected, isIE8,
    	active_class, clone_class, current_container_class, delete_clone,
    	drag_clone, drag_rate, dragged_class, dragging, options,
    	original_container_class, placeholder_class, previous_container_class,
    	selected_offset_x, selected_offset_y,
    	      _this = this;
    	    options = this.options;
    	    $container = this.$container;
    	    active_class = options.activeClass;
    	    ...
    	    drag_clone = options.dragClone;
    	    clone_class = options.cloneClass;
    	isIE8 = (jQuery.support.leadingWhitespace ==
    	false)&&(jQuery.support.boxSizing==true);
    	    $selected = $placeholder = $clone = selected_offset_y =
    	selected_offset_x = null;
    	    dragging = false;
    	    if (options.enableDrag) {
    	      $container.children("." +
    	active_class).filter(options.dragWhitelist).dragable({
    	        addClasses: false,
    	        containment: 'document',
    	        handle: options.handle,
    	        zIndex: 9999,
    	        start: function(e, ui) {
    	          ...
    	          if (drag_clone) {
    	            $clone =
    	$selected.clone(true).insertBefore($selected).addClass(clone_class);
    	          }
    	          $selected.addClass(dragged_class);
    	          selected_tag = $selected.prop("tagName");
    	if ( !isIE8 )
    	$placeholder = $("<" + selected_tag + " class='" + placeholder_class +
    	"' style='height: " + ($selected.height()) + "px; width: " +
    	($selected.width()) + "px'></" + selected_tag + ">");
    	...
    	$selected.parent().addClass(original_container_class).addClass(current_container_class);
    	        ...
    	        drag: function(e, ui) {
    	          if (!dragging && !(drag_clone && delete_clone && $("." +
    	current_container_class)[0] === $("." + original_container_class)[0])) {
    	...
    	$("." + current_container_class).trigger("ss-setTargetPosition");
    	          ...
    	          ui.position.left = e.pageX - $selected.parent().offset().left
    	- selected_offset_x;
    	          return ui.position.top = e.pageY -
    	$selected.parent().offset().top - selected_offset_y;
    	        },
    	        stop: function() {
    	          if ( isIE8 )
    	$("." + current_container_class).trigger("ss-setTargetPosition");
    	var $current_container, $original_container, $previous_container;
    	          ...
    	          $("." + placeholder_class).remove();
    	          if (drag_clone) {
    	            if (delete_clone && $("." + current_container_class)[0] ===
    	$("." + original_container_class)[0]) {
    	              $clone.remove();
    	              $("." + current_container_class).trigger("ss-rearrange");
    	            } else {
    	              $clone.removeClass(clone_class);
    	...
    	$original_container.trigger("ss-arrange").removeClass(original_container_class);
    	          $current_container.trigger("ss-arrange",
    	true).removeClass(current_container_class);
    	...
    	current_container_class).removeClass(current_container_class).addClass(previous_container_class);
    	        ...
    	        drop: function(e, selected) {
    	          var $current_container, $original_container,
    	$previous_container;
    	          if (_this.options.enableTrash) {
    	            $original_container = $("." + original_container_class);
    	            $current_container = $("." + current_container_class);
