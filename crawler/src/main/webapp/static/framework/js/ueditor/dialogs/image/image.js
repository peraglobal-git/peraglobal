...
(function () {
    ...
    OnlineImage.prototype = {
        ...
        pushData: function (list) {
            ...
            var listPath = editor.getOpt('imageManagerListPath');
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    // wenqihui update 修改路径
                    var url = list[i].url;
                    uindex=url.indexOf(listPath);
                    if(uindex!=-1){
                    	var rootPath = url.substring(0,uindex);
                    	rootPath = rootPath.substring(rootPath.lastIndexOf("/"));
                    	list[i].url =  rootPath+url.substring(uindex);
                    }
                    img.width = 113;
