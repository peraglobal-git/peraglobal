...
//				lastDataId = data.id;
	...
	saveTaskMethod = function(title,param){
		pdpDialog.openDialog({url:'../count/toChart.html?'+param,title:title,
			width:'790px',
			height:'550px',
			reloadCallBack:function(data){
				movelog=false;
	...
	exports.gridLoad = function(type){
		...
		//请求连接
		var url = '../count/queryCountList.html';
		//列
		var columns = [[
		                //要显示的列
		                ...
		                {field:'index',width:40,halign:'center',align:'center',title:'序号',
		               	...
		               	{field:'name',halign:'center',align:'left',width:300,title:'知识源名称',
		               	...
		               	{field:'taskjobCount',halign:'center',align:'center',width:120,title:'<a id="typeClick" class="typeClick" onClick="typeClick()">采集任务数</a>',
		                	formatter:function(val,row){
		                		var nameVal = val;
		                		if(nameVal==0||nameVal==""){
				                	return '<a >'+ nameVal +'</a>';
		                		}else{
				                	return '<a id="view_'+row.id+'" class="easyui-linkbutton viewDic" iconCls="icon-edit" plain="true" href="javascript:void(0)" title="点击查看详情" style="text-decoration:underline;">'+ nameVal +'</a>';
		                		}
			                }
		               	},
		                {field:'taskDataCount',width:120,
